...
from models.GPTConfig import GPT2_1B, GPT2_4B, GPT2_7B, GPT2_Tiny
from optimizers.MuON import Muon
from optimizers.lr_scheduler import LRScheduler
from dataloaders.DDP import DistributedDataLoader, save_checkpoint
from training.Hyperparams import CSCS_60GPU, CSCS_160GPU_2K, CSCS_4GPU_2K
...
def parse_args():
    ...
    parser.add_argument(
        "--model", 
        type=str, 
        choices=["1B", "4B", "7B", "tiny"],
        default="1B",
        help="Model size: 1B (~1.5B params), 4B (~4B params), 7B (~7B params), tiny (~500K params)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Use tiny model for testing (equivalent to --model tiny)"
    )
    parser.add_argument(
        "--resume",
        type=str,
    ...
    parser.add_argument(
        "--config",
        type=str,
        choices=["CSCS_60GPU", "CSCS_160GPU_2K", "CSCS_4GPU_2K"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Stop training after this many steps (useful for timing/feasibility runs)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Tensor parallel size (default: 1, no tensor parallelism)"
    )
...
def get_model_config(args):
    ...
    if args.dry_run or args.model == "tiny":
        return GPT2_Tiny()
    elif args.model == "4B":
        return GPT2_4B()
    elif args.model == "7B":
...
def main():
    ...
    parallel = build_parallel_context(args.tp_size)
    ...
    if master_process:
        print(
            f"Parallel setup: world_size={parallel.world_size}, "
            f"local_world_size={parallel.local_world_size}, "
            f"tp_size={parallel.tp_size}, dp_size={parallel.dp_size}"
        )

    print(
        f"[rank {parallel.rank}] "
        f"local_rank={parallel.local_rank} "
        f"tp_group={parallel.tp_group_ranks} "
        f"dp_group={parallel.dp_group_ranks} "
        f"tp_rank={parallel.tp_rank} "
        f"dp_rank={parallel.dp_rank}"
    )
    ...
    # Use dp_size (not world_size): TP peers process the same micro-batch together,
    # so the number of independent data consumers is dp_size = world_size // tp_size.
    assert hparams.val_tokens % (B * T * parallel.dp_size) == 0
    val_steps = hparams.val_tokens // (B * T * parallel.dp_size)

    assert hparams.batch_size % (B * parallel.dp_size) == 0
    train_accumulation_steps = hparams.batch_size // (B * parallel.dp_size)
    ...
    # dp_rank / dp_size: TP peers share the same dp_rank, so they automatically
    # read the same data shard.  DP peers have different dp_ranks and read different shards.
    train_loader = DistributedDataLoader(
        hparams.input_bin, B, T, parallel.dp_rank, parallel.dp_size, on_shard_advance, hparams.skip_files
    )
    val_loader = DistributedDataLoader(
        hparams.input_val_bin, B, T, parallel.dp_rank, parallel.dp_size, None, None
    )
    if master_process:
    ...
    model = GPTTP(model_config, tp_group=parallel.tp_group, tp_size=parallel.tp_size)
    if master_process:
        pass  # model param count printed in the training config summary below
    ...
    # process_group=dp_group: DDP only syncs gradients across DP replicas.
    # TP peers must NOT be included — they already exchange activations and
    # gradients via the all_reduces inside ColumnParallelLinear / RowParallelLinear.
    # Including them in DDP would double-count their gradients.
    # With tp_size=1, dp_group == default world group, so behaviour is unchanged.
    model = DDP(
        model, device_ids=[ddp_local_rank],
        bucket_cap_mb=256,
        gradient_as_bucket_view=True,
        process_group=parallel.dp_group,
    )
    ...
    # dp_rank / dp_size: Muon coordinates Newton-Schulz orthogonalization across
    # replicas that hold identical weight shards.  TP peers hold different shards,
    # so they must not be included in Muon's process group.
    optimizer2 = Muon(
        raw_model.transformer.h.parameters(),
        lr=0.1 * hparams.learning_rate,
        momentum=0.95,
        rank=parallel.dp_rank,
        world_size=parallel.dp_size,
    )
    ...
    if args.max_steps is not None:
        num_iterations = min(num_iterations, args.max_steps)
    if hparams.state_dict is not None:
    ...
    if master_process:
        num_params = raw_model.num_params_total()
        tokens_per_step = hparams.batch_size * hparams.sequence_length
        print(f"""
============================================================
  Training Configuration
============================================================
  Model          : {args.model} ({num_params/1e9:.2f}B params)
  Config         : {args.config}
  World size     : {ddp_world_size} GPUs
  Batch size     : {hparams.batch_size} sequences ({tokens_per_step:,} tokens/step)
  Device batch   : {hparams.device_batch_size} sequences
  Seq length     : {hparams.sequence_length}
  Grad accum     : {train_accumulation_steps}
  Learning rate  : {hparams.learning_rate}
  Warmup iters   : {hparams.warmup_iters}
  Warmdown iters : {warmdown_iters}
  Total iters    : {total_iterations}
  Max steps      : {args.max_steps if args.max_steps else "unlimited"}
  Val every      : {hparams.val_loss_every} steps
  Val tokens     : {hparams.val_tokens:,}
  Train tokens   : {train_loader.ntok_total:,} ({len(train_loader.files)} file(s))
============================================================
""")
        ...
        if last_step or (hparams.val_loss_every > 0 and step % hparams.val_loss_every == 0):
            ...
            for _ in range(val_steps):
                ...
                with ctx:
                    ...
                    del loss
            # Average only across DP peers: TP peers compute identical losses
            # (same batch, same full lm_head), so including them would skew the average.
            dist.all_reduce(val_loss, op=dist.ReduceOp.AVG, group=parallel.dp_group)
            val_loss /= val_steps
